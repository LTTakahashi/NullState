...
import numpy as np
import pandas as pd
from analyze_random_frame_null import analytic_moments, ks_test, GAP_MAX


def load():
    fs = sorted(glob.glob("results_sigma_min_s*.csv"))
    d = (pd.concat([pd.read_csv(f) for f in fs], ignore_index=True)
         if fs else pd.read_csv("results_sigma_min.csv"))
    return d.drop_duplicates(["s_level", "seed"])


def main():
    d = load()
    d.to_csv("results_sigma_min.csv", index=False)
