...
`dgp.py` and `metrics.py` are imported on purpose by `../analysis/geometry_constant.py`.
...
`core/dgp.py` + `gates/gate1_dgp.py` (Stage-1 decoupling gate), `core/metrics.py`
(rotation-sensitive MCC/CCA/DCI + oracle ceiling), `core/models.py` (genuine NB
