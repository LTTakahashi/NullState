...
([`explorations/derisk_gauge.py`](explorations/derisk_gauge.py)) looked strong (θ=0 change +0.007, θ=π/4
change +0.092, ~16×). But the 4-seed **dose-response**
([`explorations/derisk_gauge_dose.py`](explorations/derisk_gauge_dose.py)) is the honest picture:
...
[`explorations/derisk_alignment.py`](explorations/derisk_alignment.py): standard v2 DGP (no gauge), a
...
## The DGP ([`core/dgp_gauge.py`](core/dgp_gauge.py), built and smoke-tested)
...
- Power: reuse [`analysis/power.py`](analysis/power.py) / [`analysis/equivalence.py`](analysis/equivalence.py);
