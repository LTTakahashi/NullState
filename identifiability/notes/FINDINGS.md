...
([`metrics2.entanglement_gap`](core/metrics.py)) and demote the matched-noise oracle to
...
**We tested the obvious attack this implies** ([`experiments/run_basis_rotation.py`](experiments/run_basis_rotation.py)
→ [`analysis/basis_rotation.py`](analysis/basis_rotation.py); 128 runs, 8 seeds). In
...
([`core/dgp_rotated.py`](core/dgp_rotated.py)) and reran, with the faithful arm at the
...
([`analysis/identifiability_floor.py`](analysis/identifiability_floor.py), on the
...
([`experiments/run_variability_condition.py`](experiments/run_variability_condition.py)): environment-
...
law ([`analysis/random_frame_null.py`](analysis/random_frame_null.py)):
...
Measured directly ([`analysis/taxonomy.py`](analysis/taxonomy.py)): corrupt the
...
([`analysis/geometry_constant.py`](analysis/geometry_constant.py),
[`analysis/closed_form.py`](analysis/closed_form.py)). The small offset between the
...
k-th-neighbour ties ([`analysis/closed_form.py`](analysis/closed_form.py) V6:
...
The gate ([`gates/gate2_metrics.py`](gates/gate2_metrics.py), PASS) proves this metric does
...
**The DGP actually decouples the knobs** ([`gates/gate1_dgp.py`](gates/gate1_dgp.py),
...
**Recovery does not degrade with overlap** ([`experiments/run_main_sweep.py`](experiments/run_main_sweep.py) →
[`analysis/main_sweep.py`](analysis/main_sweep.py); 900 runs at 25 seeds/cell). The
...
"d<0.8" was weak evidence ([`analysis/power.py`](analysis/power.py)). We treat
...
d=0.8, not a non-significant p ([`analysis/equivalence.py`](analysis/equivalence.py)).
...
bound** ([`experiments/run_frontier.py`](experiments/run_frontier.py), [`experiments/run_mixing_bound.py`](experiments/run_mixing_bound.py)).
...
The λ sweep behind the caveats above ([`experiments/run_mixing_bound.py`](experiments/run_mixing_bound.py),
...
([`experiments/run_alignment.py`](experiments/run_alignment.py) →
[`analysis/alignment.py`](analysis/alignment.py); 25 seeds/cell). CCA (subspace
...
[`notes/DESIGN_NOTES.md`](notes/DESIGN_NOTES.md) after a 4-seed pilot and **before** the 25-seed
...
**The shape, reported honestly** ([`analysis/alignment_shape.py`](analysis/alignment_shape.py)).
...
[`notes/DESIGN_NOTES.md`](notes/DESIGN_NOTES.md); it gave only a weak effect — a faithful encoder
...
```bash
python -m analysis.taxonomy                            # THE THESIS: invariance-class table
python -m analysis.geometry_constant                     # demo 1 receipt (oracle -> 1-4^(-1/3))
python -m analysis.closed_form                            # the recipe: marginal / scale / k / rotation
python -m gates.gate1_dgp && python -m gates.gate2_metrics && python -m gates.gate3_models
python -m experiments.run_main_sweep   && python -m analysis.main_sweep     # main result (5-seed pilot)
python -m analysis.power                                # MDE: n=5 d~2.0, n=25 d~0.8
python -m experiments.run_frontier && python -m experiments.run_mixing_bound   # recovery-preserving frontier, both directions
python -m analysis.equivalence results/main_sweep.csv 0.8  # TOST vs pre-registered SESOI (needs merged n=25)
python -m experiments.run_alignment && python -m analysis.alignment  # objective-induced overlap-dependence
python -m analysis.alignment_shape                      # curve shape: monotonicity + segmented-vs-linear BIC
python -m experiments.run_basis_rotation && python -m analysis.basis_rotation  # basis-dependence of the gap
python -m analysis.identifiability_floor                # the floor as an instrument (per-arm, SESOI in gap units)
python -m experiments.run_variability_condition                    # does the iVAE condition lower the floor?
python -m core.certificate                                  # certificate validation
```
